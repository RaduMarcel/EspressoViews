...
- 	Login-Fenster erweitern, 
	SID oder Service_Name anwählen können !!
...
//			public void itemStateChanged(ItemEvent arg0) {// Wird aktiviert wenn der Benutzer in der ComboBox-Liste etwas auswählt 
//				//System.out.println("Selected Item  "+(String)(comboBoxStoredConns.getSelectedItem()) );
//				if (connectionDetails!=null) //enthält Verbingsdetails, so wie sie im ausgewählten definition file vorgefunden werden 
...
//		//In dieser ComboBox gibt man den file locator der file definition ein oder man wählt einen der locators aus, der im inisetting file vorgegeben wurde.
...
//				if ( comboBoxFileDef.getSelectedItem() !=null ){// wenn der Benutzer in der ComboBox-Liste eine definition file auswählt oder es erst eingibt und dann auswählt...
...
//				
//				if(succes)
//				{//Wenn die Loginsdetails erfolgreich angewendet wurden, dann wird ausgelotet ob etwaige Ergänzungen der Logindetails stattgefunden haben 
...
//							if (            
//								//wenn das möglicherweise manuell veränderte Dropdown-Element	gleich ist einem der geladenen Dropdown-Elementen...  
//								    ((String)(comboBoxStoredConns.getModel().getElementAt(t))).equals((String) ( comboBoxStoredConns.getModel().getSelectedItem()))
//								 || ((String) ( comboBoxStoredConns.getModel().getSelectedItem())).trim().length()==0 //...oder auch wenn der User den ConnectionNamen gelöscht hat
//							   ){//...und somit nicht verändert wurde dann Prüfe ob sich vielleicht die login-Details verändert haben
...
//										}
//									}
//									if(newConnDetailsEntered)// ...dann könnte man in der Defnitionsdatei die neuen Logindaten unter denselben ConnectionNamen speichern"
//										{
...
//										}
//									else System.out.println("die veränderten connectionDetails gibt es unter einem anderen Namen, nichts passiert.");
//								}	
...
//							//Prüfe nun die, ob connectionDetails aus den Eingabefeldern neu sind oder mir irgendeiner Connection übereinstimmen " 
//							boolean newConnDetailsEntered=true; 
//							for (Map<String,String> conns : connectionDetails){
//								if ( (host+serviceName+userName).toLowerCase().equals( (conns.get("HOST")+conns.get("SERVICENAME")+conns.get("USERNAME")).toLowerCase() ) ){
//									newConnDetailsEntered=false;
...
//									QueryDefinitionParser.amendConnectionDetailsXML(selectedFile, conns.get("CONNECTIONNAME"), newConnDet );//Änderung speichern
...
//									comboBoxStoredConns.addItem( (String)(comboBoxStoredConns.getModel().getSelectedItem()));//und der Neue hinzugefügt
//									break;
//								}
//							}
